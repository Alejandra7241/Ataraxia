...
function modal_for_subject(code,name,credits,typology, array_prerequisites){
    ...
    var index; var auxindex;
    for (index = 0; index < array_of_subjects.length; ++index) {
        var current_subject = array_of_subjects[index].split(",");
        for (auxindex = 0; auxindex < current_subject.length; ++auxindex) {
            console.log(current_subject[auxindex]);
        }
    }
    ...
    $('#tablePrerrequisitosSubjectModal > tbody:last-child').append(' <tr><td>' + rr[0]+ '</td><td>' + rr[1] + '</td><td class="text-right">' + rr[3]  + '</td><td> ' + postLink + '</td></tr>');
    }else{

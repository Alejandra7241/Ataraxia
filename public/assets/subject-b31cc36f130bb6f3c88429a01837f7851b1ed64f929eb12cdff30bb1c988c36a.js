...
function modal_for_subject(code,name,credits,typology, array_prerequisites, array_postrequisites, role, code_career, array_optatives){
    ...
    if(!typology == 'O')document.getElementById("subjectData").innerHTML = '<h4 class = "text-left"><b>Código: </b>' + code + '</h4>' + '<h4 class = "text-left"><b>Creditos: </b>' + credits + '</h4>';
    ...
    if(array_postrequisites.length == 0){
        ...
        for (index = 0; index < array_of_subjects.length; ++index) {
            ...
            loadPreAndPostLink =  `  <form action="/student/load_change" method="post" enctype="multipart/form-data" class="form-horizontal">
                    <button type="submit">
                    <input type="hidden" name="code" id="setCode" value="` + current_code +`" />
                    <input type="hidden" name="typology" id="setTypology" value="` + current_typology +`" />
                    <input type="hidden" name="code_to_remove" id="setTypology" value="` + code +`" />
            ...
            else $('#tablePostrrequisitosSubjectModal > tbody:last-child').append( firsttr + '<td>' + current_code+ '</td><td>' + current_name + '</td><td class="text-right">' + current_credits  + '</td><td> ' + loadPreAndPostLink + '</td></tr>');
        }
    }
    ...
    if(array_optatives.length == 0){
        ...
        for (index = 0; index < array_of_subjects.length; ++index) {
            ...
            else $('#tableOptativasSubjectModal > tbody:last-child').append( firsttr + '<td>' + current_code+ '</td><td>' + current_name + '</td><td class="text-right">' + current_credits  + '</td></tr>');
        }
    }
